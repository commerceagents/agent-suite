...
export default function Navigation({ show = true, delay = 0 }) {
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center gap-4 md:gap-8 lg:gap-10 backdrop-blur-2xl border border-white/10 rounded-2xl pl-3 md:pl-5 pr-3 py-1 shadow-[0_8px_32px_rgba(0,0,0,0.4)] ring-1 ring-white/10"
          style={{
        ...
        >
          ...
          <motion.div 
            variants={item} 
            className="flex items-center group -ml-1 md:-ml-2 pr-4 md:pr-6"
          >
            ...
            {navItems.map((link) => {
              ...
              return (
                <motion.div 
                  key={link.name}
                  variants={item} 
                  className={`relative py-1.5 lg:py-2 group cursor-pointer ${link.name === 'Contact us' ? 'px-4 lg:px-6 ml-1 md:ml-2' : 'px-2 lg:px-3'}`}
