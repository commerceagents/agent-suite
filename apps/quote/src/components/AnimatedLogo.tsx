...
export default function AnimatedLogo({
  ...
  const rep = loop ? { repeat: Infinity, repeatDelay: 3.5 } : {};

  return (
    <div className={`relative ${SIZE_MAP[size]} ${className}`}>
      <svg viewBox="0 0 198 198" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
        <defs>
          ...
          <mask id="iconShimmerMask">
            <motion.rect
              ...
              animate={{ x: [-400, 400] }}
              transition={{ delay: delay + shimmerDelay, duration: 2.0, ease: "easeInOut" }}
            />
          </mask>
        </defs>

        {/* 1 — Star trace */}
        <motion.path d={PATHS.star} stroke="rgba(192,192,200,0.6)" strokeWidth="1.5" fill="none"
          initial={{ pathLength: 0, opacity: 0 }}
          animate={{ pathLength: 1, opacity: [0, 0.8, 0.8, 0] }}
          transition={{ delay: delay + 0.2, duration: 1.2, ease: "easeInOut", ...rep }}
        />
        {/* 2 — Outer C trace */}
        <motion.path d={PATHS.outerC} stroke="rgba(192,192,200,0.5)" strokeWidth="1.5" fill="none"
          initial={{ pathLength: 0, opacity: 0 }}
          animate={{ pathLength: 1, opacity: [0, 0.7, 0.7, 0.2] }}
          transition={{ delay: delay + 0.6, duration: 1.4, ease: "easeInOut", ...rep }}
        />
        {/* 3 — Inner shape trace */}
        <motion.path d={PATHS.inner} stroke="rgba(192,192,200,0.4)" strokeWidth="1" fill="none"
          initial={{ pathLength: 0, opacity: 0 }}
          animate={{ pathLength: 1, opacity: [0, 0.6, 0.6, 0.2] }}
          transition={{ delay: delay + 0.8, duration: 1.2, ease: "easeInOut", ...rep }}
        />
        {/* 4 — Outer C fill */}
        <motion.path d={PATHS.outerC} fill="url(#chromeGrad)"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          transition={{ delay: delay + 1.6, duration: 0.5, ease: "easeOut", ...rep }}
        />
        {/* 5 — Inner fill */}
        <motion.path d={PATHS.inner} fill="url(#chromeGrad)"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          transition={{ delay: delay + 1.7, duration: 0.5, ease: "easeOut", ...rep }}
        />
        {/* 6 — Square snap */}
        <motion.rect x="143.927" y="142.277" width="12.8228" height="12.8228" fill="url(#chromeGrad)"
          initial={{ scale: 0, opacity: 0 }}
          animate={{ scale: [0, 1.2, 1], opacity: 1 }}
          transition={{ delay: delay + 2.0, duration: 0.4, ease: [0.34, 1.56, 0.64, 1] as const, ...rep }}
        />
        {/* 7 — Shimmer */}
        <motion.g mask="url(#iconShimmerMask)" style={{ mixBlendMode: "overlay", opacity: 0 }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          transition={{ delay: delay + shimmerDelay, duration: 0.01, ...rep }}
        >
          <path d={PATHS.outerC} fill="white" />
          <path d={PATHS.inner} fill="white" />
